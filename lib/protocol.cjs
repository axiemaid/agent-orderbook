 ...
 * Handles: PLACE, FILL, CANCEL, DELIVER, DISPUTE, REFUND, TIMEOUT, TASK
 */
function decodeOrd1(scriptHex) {
  ...
  try {
    switch (action) {
      ...
      case 'BOUNTY': {
        // "ORD1" "BOUNTY" <type:1B> <reward:8B LE> <expiry:4B LE> <task:var>
        result.type = items[2][0]
        result.reward = Number(items[3].readBigUInt64LE())
        result.expiryHeight = items[4].readUInt32LE()
        result.task = items[5]?.toString('utf8') || ''
        break
      }
      case 'CLAIM': {
        // "ORD1" "CLAIM" — fixed receipt (answer is in input script, not OP_RETURN)
        // Indexers extract bounty_txid + answer from the input witness
        result.bountyTxid = items[2]?.toString('hex') || ''
        result.answer = items[3]?.toString('utf8') || ''
        break
      }
      case 'TASK': {
        // Legacy format from agent-a-post.cjs
        result.type = items[2][0]
        if (items.length >= 5) {
          result.question = items[4].toString('utf8')
        } else if (items.length === 4) {
          result.question = items[3].toString('utf8')
        }
        break
      }
      case 'REFUND':
