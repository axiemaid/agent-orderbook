...
function decodeOrd1(scriptHex) {
  ...
  try {
    switch (action) {
      ...
      case 'TASK': {
        result.type = items[2][0]
        // items[3] is a 1-byte push of question length, items[4] is the question
        // But the format from agent-a-post.cjs is: 01 <type:1B> 01 <qLen:1B> <question>
        // So items[2] = type (1 byte), items[3] = qLen (1 byte), items[4] = question
        // Actually looking at the builder: push(type 1B), push(qLen 1B), push(question)
        // So items[2] = type, items[3] = qLen, items[4] = question bytes
        // But qLen is pushed as a separate pushdata item
        if (items.length >= 5) {
